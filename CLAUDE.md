...
A single-binary CLI that replaces the hand-edited `DEVLOG.md` working channel used by the OpenSpec
four-role agent workflow. Written in Zig 0.16 with no third-party dependencies, it keeps an append-only
log of attributed records in `DEVLOG.jsonl` — the tool's only state — and derives everything else in
memory on each invocation. Its consumer is the `dmons` plugin in the sibling `dmon-dev` repository.
...
**Note the bootstrap:** this project exists to replace that hand-edited Markdown file with a tool.
...
1. **Brief the worker.** Post the brief to the DEVLOG (`[architect]`, under the block's `## N.`
   section): the block's tasks (`N.1`…`N.k`), the relevant spec excerpts, the binding ADRs and design
   decisions that bind them, and the done-gates below. The worker shouldn't need to go hunting.
2. **Worker implements the block** and reports back, posting to the DEVLOG as it goes. If the boundary
   tripwire fired on its return, deal with that **before** step 3 — an unreviewed, ungated commit is not
   a starting point for a review.
3. **Audit.** Spawn `reviewer` on the block diff (correctness, ADR compliance, OpenSpec scope, Zig
   idiom, log-format integrity, stdin handling, and Zig memory discipline). The reviewer posts its
...
- a task **requires human-in-the-loop verification** that can't be settled by automated gates — e.g.
  confirming the binary refuses to hang when stdin is a real terminal, judging whether a real
  `DEVLOG.jsonl` diff is tolerable to read, checking the shipped binary size and that `otool -L` / `ldd`
  shows no third-party linkage, or validating cross-compiled release tarballs on a real machine.
