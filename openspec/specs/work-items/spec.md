# Work Items Specification

## Purpose

Defines the work items agents raise in the log: questions, findings, decisions, notes, and tasks, each
optionally addressed to a role and independently flagged as blocking, each given a short quotable
identifier, and each closed by a separate close record written only by a declared closing role.
