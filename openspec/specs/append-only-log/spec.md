# Append-Only Log Specification

## Purpose

Defines the log as an append-only sequence of attributed records: nothing already written is ever
modified or deleted, every record names the role that wrote it and the work it concerns, bodies arrive
as Markdown on standard input, and records carry a definite order. Corrections are expressed by
appending further records rather than by rewriting history.
